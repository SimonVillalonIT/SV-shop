...
const CartIcon = ({ fill, className }: CartIconInterface) => {
  const { items } = useCartStore();

  return (
    <div
      className={cn(
        "bg-gradient-to-br relative from-[#9BA7F9] to-[#FC8CD6] p-2 rounded-xl flex justify-center items-center cursor-pointer",
        className,
      )}
    >
      <span className="rounded-full font-bold text-black absolute bg-white text-xs -top-1 -right-1 px-1 select-none">
        {items.length}
      </span>
      <svg
